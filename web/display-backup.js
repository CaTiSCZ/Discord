...
socket.on("new_message", (data) => {
    ...
    if (el) {
        ...
        el.innerHTML = ""; // Vyčistíme předchozí obsah
        data.messages.forEach(msg => {
            const messageBlock = document.createElement("div");
            messageBlock.className = "message-entry";
            const finalWidth = settings.column_width || settings.width || 600;
            // Dynamické stylování buňky
            //messageBlock.style.maxWidth = (settings.column_width || settings.width || 600) + "px"; // Maximální šířka pro zalamování
            messageBlock.style.width = "max-content"; // Nastaví šířku pro správné zalamování
            //messageBlock.style.minWidth = finalWidth + "px"; // Zabrání smrštění (ořezu)
            messageBlock.style.maxWidth = finalWidth + "px"; // Zabrání přetečení
            //messageBlock.style.flexShrink = "0"; // Zabrání zmenšování pod min-width
            messageBlock.style.display = "block"; // Umožní nastavit šířku a zalamovat text
            //messageBlock.style.flex = "0 0 auto"; // Zabrání zmenšování a roztahování
            //messageBlock.style.breakInside = "avoid-column"; // Zabrání rozdělení zprávy mezi sloupci
            //messageBlock.style.pageBreakInside = "avoid";    
            messageBlock.style.marginBottom = "10px"; // Mezera mezi zprávami pod sebou
            //messageBlock.style.marginRight = "20px";  // Mezera mezi sloupci
            messageBlock.style.whiteSpace = "normal"; // Zachová mezery a umožní zalamování
            messageBlock.style.overflowWrap = "anywhere"; // Umožní zalamování i u dlouhých slov
            messageBlock.style.wordBreak = "break-word";

            messageBlock.style.boxSizing = "border-box"; // Zahrne padding a border do celkové šířky
            

            messageBlock.innerHTML = addAuthor(msg);
            el.appendChild(messageBlock);
        });
        if (globalConfig.panels && globalConfig.panels[data.panel]) {
            applyBackground(data.panel, settings);
        }
    } else {
...
function addAuthor(msgData) {
    ...
    const mode = msgData.show_author || "both";
...
function applyPanelStyle(id, settings) {
    ...
    if (settings.is_image) {
        ...
        const colWidth = settings.column_width || settings.width || 600;
        content.style.column = colWidth + "px"; // Nastaví počet sloupců a jejich šířku
        content.style.columnGap = "20px"; // Mezera mezi sloupci
        content.style.columnFill = "auto"; // Vyplní sloupce postupně, ne rovnoměrně
        content.style.display = "block"; // Umožní nastavit šířku a zalamovat text
        //content.style.flexDirection = "column";
        //content.style.flexFlow = "column wrap";
        //content.style.flexWrap = "wrap";
        //content.style.alignContent = "flex-start";
        //content.style.boxSizing = "border-box";
        content.style.height = settings.height + "px";
        //content.style.maxHeight = settings.height + "px";
        //content.style.width = "max-content";

        content.style.width = "fit-content"; // Zajistí, že obsah zabere celou šířku panelu a umožní zalamování
        content.style.minWidth = "0"; // Zabrání smrštění pod šířku panelu
        content.style.maxWidth = "none";
        content.style.overflow = "hidden"; // Skryje přetečení, ale umožní zalamování
        el.style.overflow = "hidden"; // Skryje přetečení, ale umožní zalamování
